        ...
        >
          <div v-if="currentProjectInfo" class="flex bg-surface-2 rounded-t-xs mt-1">
            <div
              ...
              class="w-45 mb-px flex items-center shrink-0 max-w-full border border-transparent rounded-xs hover:border-primary focus-within:border-primary-variant focus-within:hover:border-primary-variant"
            >
              ...
              <svg
                ...
                viewBox="0 0 24 24"
                height="24px"
                class="mx-1 fill-primary-variant"
              >
                <path
              ...
              <input
                ...
                class="flex-1 bg-transparent outline-none min-w-0 p-0.5 truncate text-primary-variant"
                @keydown.enter="handleProjectEnter"
                @focusout="handleRenameProject"
              />
            </div>

            <button
              type="button"
              @click="projectManager.closeCurrentProject"
              :disabled="stateManager.isSaving.value"
              :title="stateManager.isSaving.value ? 'Saving...' : 'Close project'"
              class="relative h-full px-3 mb-px flex items-center justify-center text-xl rounded-xs border border-transparent text-primary-variant"
              :class="
                stateManager.isSaving.value
                  ? ''
                  : 'hover:text-on-surface hover:bg-error hover:border-on-surface'
              "
            >
              <span
                class="-translate-y-px"
                :class="stateManager.isSaving.value ? 'text-transparent' : ''"
                aria-hidden="true"
              >
                ×
              </span>
              ...
              <div
                v-if="stateManager.isSaving.value"
