<template>
  <div class="flex flex-col h-screen bg-surface-1 text-on-surface">
    ...
    <div class="flex flex-col flex-1 items-center justify-start px-6 py-8 overflow-y-auto gap-4">
      <div class="max-w-md text-center mt-4">
        ...
        <p class="text-lg mb-4">
          LogicEasy is designed for larger screens. Your current screen resolution is not supported.
          Large parts of the user interface will likely not be usable.
        </p>

        <!-- Suggestion -->
        <div class="bg-surface-2 bg-opacity-10 rounded-lg p-4 mb-4">
          <p class="text-sm">
            You can try <strong>zooming</strong> out your browser to fit the content on your screen.
            You can use <code>Ctrl + Minus</code> (<code>Cmd + Minus</code> on Mac) to zoom out.
          </p>
        </div>
        ...
        <div class="bg-surface-2 rounded-lg p-4 mb-6">
          ...
          <p class="text-lg font-semibold text-primary">{{ minWidth }} × {{ minHeight }} pixels</p>
          <p class="text-sm mt-2">
            <strong>Current Resolution:</strong>
          </p>
          <p class="text-base">{{ currentWidth }} × {{ currentHeight }} pixels</p>
        </div>
      </div>
    </div>
    ...
    <div class="shrink-0 flex justify-center px-6 py-8 border-t border-surface-2">
      ...
      <p class="text-xs text-gray-500 ml-4 flex items-center">⚠️ Not recommended</p>
    </div>
  </div>
</template>
