  ...
  <div class="flex items-center relative h-[calc(100vh-52px)] m-4 ml-12 text-on-surface">
    ...
    <div class="absolute flex justify-end items-end w-full h-full">
      <a class="bg-surface-2 hover:bg-surface-3 p-0! mb-4 rounded-full shadow-2xl"
        ...
        <img src="/logic-easy/GithubLogo.png" alt="GitHub Logo" class="w-12 aspect-auto invert">
      </a>
    </div>
    ...
    <div class="absolute rounded-xs bg-elevated p-10 pr-[20%] shadow-2xl">
      ...
      <div class="flex flex-col gap-4 mt-6">
        <button v-for="(item, idx) in items" :key="idx" @click="runAction(item)" :disabled="!item.panelKey"
          class="flex gap-2 p-2 hover:text-secondary-variant underline-offset-4 hover:underline disabled:bg-transparent disabled:no-underline disabled:text-on-surface-disabled rounded-xs cursor-pointer disabled:cursor-default"
          :title="item.label">
