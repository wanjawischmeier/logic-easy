<template>
  <BaseDropdownButton title="Settings">
    <template #icon>
      <svg
    ...
    <template #content>
      <div class="flex flex-col gap-3 min-w-76">
        ...
        >
          <h3 class="text-xs opacity-70 text-on-surface">{{ label }}</h3>
          <slot v-if="$slots[slotName]" :name="slotName"></slot>
        </div>

        <div v-if="showOutputVarSelector && showOutputSelection" class="flex flex-col gap-1">
          <label class="text-xs opacity-70 text-on-surface">Output Variable</label>
          <MultiSelectSwitch
            :values="outputVars"
        ...
        <div v-if="showFunctionTypeSelection" class="flex flex-col gap-1">
          <label class="text-xs opacity-70 text-on-surface">Function Type</label>
          <MultiSelectSwitch
            :values="Object.values(FunctionType)"
        ...
        <div v-if="showFunctionRepresentationSelection" class="flex flex-col gap-1">
          <label class="text-xs opacity-70 text-on-surface">Representation</label>
          <MultiSelectSwitch
            :values="Object.values(FunctionRepresentation)"
...
const selectedFunctionRepresentationIndex = computed(() =>
  Object.values(FunctionRepresentation).indexOf(
    props.selectedFunctionRepresentation ?? defaultFunctionRepresentation,
  ),
)
