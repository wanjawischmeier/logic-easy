...
The main purpose of the fsm project type is to keep fsm behavior consistent across panels and the external FSM editor. This includes all of the following goals:

- Provide one shared fsm data model.
- Coordinate sync between app state and FSM iframe.
- Expose derived fsm data for UI modules.
- Prevent sync loops and redundant updates.

## Structure

- `FsmProject.ts` -> main domain entry for fsm projects.
  - owns editor/app synchronization behavior.
  - exposes reactive fsm state via `useState()`.
  - handles normalization, listener lifecycle, and project hooks.

- `FsmTypes.ts` -> type definitions for fsm data.
  - includes state, transition, binary row, and sync-related shapes.

- `FsmPropsComponent.vue` -> Project creation/config UI for fsm-specific properties.
...
- 'fsm state' and 'state'
  -> The term 'fsm state' as used in the main module refers to the central project state, whereas the term 'state' is also used to label the state nodes of the fsm (e.g. the initial state in which the fsm starts its processses)
- 'fsm type' and '... type'
  -> Since we implemented this project mainly in TS, the term 'type' is used to define data models. E.g. the file 'FsmTypes.ts' therefore includes all these definitions. In contrast to that, the term 'fsm type' is also used to label the variable which states the type ['mealy' | 'moore'] of one specific fsm.

## Fsm Synchronization

This page describes the fsm data flow between the editor, the table view, the logic circuits iframe, and the central (saved) project state.
The goal is to help contributors understand behavior, avoid sync loops, and reason about side effects.

### Core model

There is one shared fsm model in app state, therefore all relevant fsm data will be saved in it. The main goal is to handle the data flow in this one central unit and not e.g. in the (external) fsm engine.
Accordingly, all UI parts read from it, and all relevant edits shall write back to it.

#### Data Flow 1: Editor to app

The FSM editor iframe does not reload after each change. Instead, the app sends and receives incremental data messages so that fsm engine and fsm state are synced in case of real changes.
When a user edits the fsm inside the FSM editor, the editor sends an export message. Then, the app ...
...
2. parses and normalizes the payload of the event message
   -> goal: to export data structured according to the fsm state model, so that we did not have to change the whole fsm engine data/stores models (throttled and deduplicated sync).
3. compares it with the last imported snapshot
   -> goal: to analyze whether the event message includes new and important changes which must lead to fsm state data updates.
4. updates shared fsm state only if something really changed
...
When fsm state changes in the app, the editor is updated through a message. The app then:

1. waits briefly (short debounce) to combine rapid edits.
   -> goal: to prevent update times but also to prevent weird-looking / incomplete changes and sync-loops with the central fsm state.
...
Edits from the state/transition table directly modify the shared fsm model.
Typical actions:
...
#### Flow 4: KV diagram edits (in fsm mode)

In fsm mode, the app maps edits in kv cells back to fsm transitions and updates the shared fsm model (which leads to app-to-editor synchronization as usual).
This write-back only happens when fsm mode and export-back settings are enabled. Otherwise, other project settings shall be applied.

#### Data Flow 5: Fsm to KV projection

KV data is derived from fsm transitions (which include from- and to-'state nodes'). The app computes a truth-table like projection from them and then derives minimization/formula data for the currently displayed output (minimized function and possibly kv diagram). As in the TruthTable modules, the computed minimized function (latex formula) is always shown.
...
Treat this folder as the single fsm domain layer.
