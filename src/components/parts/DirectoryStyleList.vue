        ...
        <div v-for="(entry, idx) in entries" :key="idx" class="contents">
          ...
          <div class="h-10 flex items-center">
            <button @click="entry.action()" :disabled="entry.disabled"
              class="group flex items-center hover:text-secondary-variant underline-offset-4 disabled:bg-transparent disabled:no-underline disabled:text-on-surface-disabled rounded-xs cursor-pointer disabled:cursor-default w-full text-left"
              :title="entry.label">
              <span class="group-hover:underline">{{ entry.label }}</span>
              <span v-if="entry.subtitle" class="ml-2 text-on-surface-disabled group-hover:no-underline">{{
                entry.subtitle }}</span>
            </button>
          </div>
        </div>
